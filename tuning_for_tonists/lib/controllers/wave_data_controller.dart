...
class WaveDataController extends GetxController {
  ...
  RxList<double> waveData = RxList.filled(4096, 0);
  ...
  void setNumberOfWaveData() {
    if (waveData.length > 4096) {
      // if (waveData.length > micTechnicalDataController.bufferSize * 2) {
      if (kDebugMode) {
        print('waveData length exceeded ${4096}: ${waveData.length}');
      }
      waveData = waveData.sublist(waveData.length - 4096).obs;
      // waveData = waveData
